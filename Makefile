...
$(BUILD)managed_heap_test: test/managed_heap_test.c $(LIB)managed_heap.c \
                              $(LIB)managed_heap.h $(LIB)managed_ref.h \
                              $(LIB)segregated_heap.c \
                              $(LIB)segregated_heap.h $(LIB)pools.h \
                              $(LIB)error_sink.c | $(BUILD)
	$(CC) $(CFLAGS) -DRIFT_MANAGED_TEST -DRIFT_ALLOCATOR_TEST -o $@ \
		test/managed_heap_test.c $(LIB)managed_heap.c \
		$(LIB)segregated_heap.c $(LIB)error_sink.c

$(BUILD)managed_ref_static_link_test: test/managed_ref_static_link_test.c \
                                        $(LIB)managed_ref.h | $(BUILD)
	$(CC) $(CFLAGS) -o $@ test/managed_ref_static_link_test.c

test-managed-handles: $(BUILD)managed_heap_test \
                      $(BUILD)managed_ref_static_link_test
	$(BUILD)managed_heap_test
	$(BUILD)managed_ref_static_link_test
	@if nm -u $(BUILD)managed_ref_static_link_test | \
		grep -Eq 'managed_heap|rift_heap|rift_pools|rift_arena'; then \
